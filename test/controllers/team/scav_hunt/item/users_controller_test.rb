...
class Team::ScavHunt::Item::UsersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should assign user" do
    ...
    params = {item_user: {user_id: @tsh.team.team_users.where.not(user_id: @item.item_users.map(&:user_id)).find_by(approved: true).user_id}}
    assert_form_params params
    post team_scav_hunt_item_users_url(@tsh, *@item.for_url), params: params
